...
class Memory(object):
    ...
    def get_response_by_text(self,text,phrase_id):
        text = '%' + text + '%'
        session.execute("SELECT * FROM response WHERE response LIKE ? AND phrase_id=?",[text,phrase_id])
        return self.zip_response(session.fetchone())

    def get_phrases_by_response(self, resp):
        session.execute("SELECT phrase_id FROM response WHERE respose=?",[resp])
        phrase_ids = [p[0] for p in session.fetchall()]

        session.execute("SELECT * from phrase WHERE id IN (?)"
